...
export default function PrincipalClassHomework() {
    ...
    const handleSaveHomework = async () => {
        ...
        try {
            ...
            if (editingId) {
                ...
                Toast.show({ type: 'success', text1: 'Updated', text2: 'Homework updated and students notified' });
            } else {
                await axios.post(
                ...
                Toast.show({ type: 'success', text1: 'Success', text2: 'Homework added and students notified' });
            }
    ...
    const styles = useMemo(() => StyleSheet.create({
        ...
        homeworkCard: { backgroundColor: theme.card, borderRadius: 22, padding: 20, marginBottom: 15, borderWidth: 1, borderColor: theme.border, elevation: 2 },
        subjectHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, paddingBottom: 10, borderBottomWidth: 1, borderBottomColor: theme.border + '50' },
        ...
        cardFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 15, paddingTop: 12, borderTopWidth: 1, borderTopColor: theme.border + '30' },
    ...
    const renderStudentList = () => {
        ...
        if (data.length === 0) return (
            <View style={{ alignItems: 'center', marginTop: 50 }}>
                ...
                <Text style={{ color: theme.textLight, marginTop: 10 }}>{activeTab === 'tick' ? "No one completed yet" : "Everyone has completed!"}</Text>
            </View>
        );
        return (
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView style={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => {setRefreshing(true); fetchHomework();}} colors={[theme.primary]} />}>
                ...
                )) : <View style={styles.emptyBox}><Ionicons name="document-text-outline" size={60} color={theme.border} /><Text style={styles.emptyText}>No homework for this date</Text></View>}
            ...
            <Modal visible={showCompletions} transparent animationType="slide" onRequestClose={() => setShowCompletions(false)}>
                <View style={styles.sheetOverlay}>
                    ...
                    <View style={styles.sheetContent}>
                        <View style={styles.sheetHeader}><View style={styles.sheetHandle} /><Text style={styles.sheetTitle}>Submission Status</Text><Text style={{ fontSize: 12, color: theme.textLight, marginTop: 5 }}>{selectedHW?.subject}</Text></View>
                        <View style={styles.tabRow}>
                            <TouchableOpacity style={[styles.tab, activeTab === 'tick' && styles.activeTabTick]} onPress={() => setActiveTab('tick')}><Ionicons name="checkmark-circle" size={20} color={activeTab === 'tick' ? '#fff' : theme.success} /><Text style={[styles.tabText, activeTab === 'tick' && styles.activeTabText]}>Done ({completionData.done.length})</Text></TouchableOpacity>
                            <TouchableOpacity style={[styles.tab, activeTab === 'cross' && styles.activeTabCross]} onPress={() => setActiveTab('cross')}><Ionicons name="close-circle" size={20} color={activeTab === 'cross' ? '#fff' : theme.danger} /><Text style={[styles.tabText, activeTab === 'cross' && styles.activeTabText]}>Pending ({completionData.pending.length})</Text></TouchableOpacity>
                        </View>
                        {renderStudentList()}
                    </View>
                </View>
            </Modal>
            <TouchableOpacity style={styles.fab} onPress={() => { setSubject(''); setContent(''); setEditingId(null); setModalVisible(true); }}><Ionicons name="add" size={32} color="#fff" /></TouchableOpacity>
