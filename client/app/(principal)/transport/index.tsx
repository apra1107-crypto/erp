...
export default function TransportDashboard() {
    ...
    const handleDeleteBus = (id: number) => {
        Alert.alert('Delete Bus', 'Are you sure you want to delete this bus and all its route data?', [
            ...
            { 
                ...
                onPress: async () => {
                    try {
                        ...
                        Toast.show({ type: 'success', text1: 'Deleted', text2: 'Bus removed successfully' });
                        fetchBuses();
                    } catch (error) {
    ...
    const renderBusCard = ({ item }: { item: any }) => (
        <Animated.View entering={FadeInUp} layout={Layout.springify()} style={[styles.busCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
            ...
            <TouchableOpacity 
                ...
                onPress={() => router.push(`/(principal)/transport/live/${item.id}`)}
            >
                {item.start_point && item.end_point ? (
                    ...
                    <View style={styles.noRouteBox}>
                        ...
                        <Text style={styles.noRouteText}>Route configuration pending. Tap footer to setup.</Text>
                    </View>
                )}
            </TouchableOpacity>
            ...
            <TouchableOpacity 
                ...
                onPress={() => router.push(`/(principal)/transport/config/${item.id}`)}
            >
                <Text style={[styles.configText, { color: theme.primary }]}>Configure Route, Stops & Students</Text>
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContainer}>
                        <View style={styles.modalHeader}>
                            <Text style={styles.modalTitle}>{editingBusId ? 'Edit Bus Details' : 'Register New Bus'}</Text>
                        ...
                        <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={false}>
                            <View style={styles.inputGroup}>
                                ...
                                <TextInput style={styles.input} placeholder="Ex: MH 12 AB 1234" value={formData.busNumber} onChangeText={(v) => setFormData({...formData, busNumber: v})} placeholderTextColor={theme.textLight} />
                            </View>

                            <View style={styles.formRow}>
                                <View style={[styles.inputGroup, { flex: 1 }]}>
                                    <Text style={styles.label}>Driver Name</Text>
                                    <TextInput style={styles.input} placeholder="Full Name" value={formData.driverName} onChangeText={(v) => setFormData({...formData, driverName: v})} placeholderTextColor={theme.textLight} />
                                </View>
                                <View style={[styles.inputGroup, { flex: 1 }]}>
                                    <Text style={styles.label}>Driver Mobile</Text>
                                    <TextInput style={styles.input} placeholder="+91" keyboardType="phone-pad" value={formData.driverMobile} onChangeText={(v) => setFormData({...formData, driverMobile: v})} placeholderTextColor={theme.textLight} />
                                </View>
                            </View>

                            <Text style={styles.dividerLabel}>Staff Members (Conductors/Helpers)</Text>
                            {formData.staff.map((member, index) => (
                                <View key={index} style={styles.staffInputRow}>
                                    <TextInput style={styles.staffInput} placeholder="Name" value={member.name} onChangeText={(v) => handleStaffChange(index, 'name', v)} placeholderTextColor={theme.textLight} />
                                    <TextInput style={styles.staffInput} placeholder="Mobile" keyboardType="phone-pad" value={member.mobile} onChangeText={(v) => handleStaffChange(index, 'mobile', v)} placeholderTextColor={theme.textLight} />
                                    {formData.staff.length > 1 && (
                                        <TouchableOpacity style={styles.removeStaffBtn} onPress={() => handleRemoveStaff(index)}>
                                            <Ionicons name="close" size={18} color="#F44336" />
                                        </TouchableOpacity>
                                    )}
                                </View>
                            ))}
                            <TouchableOpacity style={styles.addStaffLineBtn} onPress={handleAddStaff}>
                                <Ionicons name="add-circle" size={20} color={theme.primary} />
                                <Text style={styles.addStaffLineText}>Add another person</Text>
                            </TouchableOpacity>
                        </ScrollView>
                        ...
                        <View style={styles.modalFooter}>
                            ...
                            <TouchableOpacity style={styles.confirmBtn} onPress={handleSaveBus}>
                                <Text style={styles.btnText}>{editingBusId ? 'Update Bus' : 'Register Bus'}</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </View>
