        ...
        <View style={[styles.container, { paddingTop: insets.top }]}>
            ...
            <FlatList 
                ...
                renderItem={({ item, index }) => {
                    ...
                    // Find the last marked time for this stop from logs
                    const stopLogs = logs.filter(l => l.stop_id === item.id && l.status === 'boarded');
                    const lastLog = stopLogs.length > 0 ? [...stopLogs].sort((a, b) => new Date(b.marked_at).getTime() - new Date(a.marked_at).getTime())[0] : null;
                            ...
                            <View style={styles.stopHeader}>
                                ...
                                <View style={{ flex: 1 }}>
                                    ...
                                    <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4, gap: 8 }}>
                                        <View style={{ backgroundColor: theme.primary + '10', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6 }}>
                                            <Text style={{ fontSize: 10, color: theme.primary, fontWeight: '800' }}>{stopStudents.length} Students</Text>
                                        </View>
                                        {lastLog && (
                                            <Text style={{ fontSize: 10, color: theme.textLight, fontWeight: '600' }}>
                                                Cleared at {new Date(lastLog.marked_at).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}
                                            </Text>
                                        )}
                                    </View>
                                </View>
