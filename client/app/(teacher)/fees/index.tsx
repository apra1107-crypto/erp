    ...
    const stats = useMemo(() => {
        ...
        const counts = {
            ...
            paid: filtered.filter(s => {
                const unpaidExtraChargesSum = (s.extra_charges || [])
                ...
                // If the record is marked 'paid' in DB, or if amount matches due
                return s.fee_status === 'paid' || (paidAmount >= totalDue && totalDue > 0);
            }).length,
            unpaid: filtered.filter(s => {
                ...
                return s.fee_status !== 'paid' && paidAmount < totalDue;
            }).length
        };
        ...
        const finalFiltered = filtered.filter(s => {
            ...
            const totalDue = baseDue + unpaidExtraChargesSum;
            const paidAmount = parseFloat(s.amount_paid || 0);
            const isFullyPaid = s.fee_status === 'paid' || (paidAmount >= totalDue && totalDue > 0);

            return filters.status === 'paid' ? isFullyPaid : !isFullyPaid;
        });

        const totalExpected = filtered.reduce((sum, s) => {
            // Expected for the month: Base Due + ALL extra charges (whether paid or not)
