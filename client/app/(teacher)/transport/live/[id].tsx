...
export default function TeacherLiveTracking() {
    ...
    const fetchData = async () => {
        try {
            const token = await AsyncStorage.getItem('teacherToken') || await AsyncStorage.getItem('token');
            const storedSessionId = await AsyncStorage.getItem('selectedSessionId');
            const tData = await AsyncStorage.getItem('teacherData');
            const teacher = tData ? JSON.parse(tData) : null;
            ...
            const [busRes, logsRes] = await Promise.all([
                ...
                axios.get(`${API_ENDPOINTS.TRANSPORT}/logs/${id}`, {
                    params: { date: dateStr, type: activeTab },
                    headers
                })
            ]);
    ...
    const handleShare = async () => {
        try {
            // Using the API base URL but pointing to the driver web route
            // In production this would be the frontend domain
        ...
        <View style={[styles.container, { paddingTop: insets.top }]}>
            <View style={styles.header}>
                ...
                <View style={{ flexDirection: 'row', gap: 10 }}>
                    <TouchableOpacity style={styles.shareBtn} onPress={() => setShowDatePicker(true)}>
